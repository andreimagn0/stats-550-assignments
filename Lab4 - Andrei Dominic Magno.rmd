...
```{r rand, echo=FALSE}
# generate random variates according to the RAND generator
# X_{n+1} = 16807 X_n mod (2^31 - 1)
...
# histogram of the n RAND variates
hist(x, main="", xlab="RAND variates", ylab="Frequencies")

# empirical distribution function of the n RAND variates and
# uniform(0, 1) probability plot
plot.ecdf(x, verticals= TRUE, do.p = FALSE, main="", ylab="Probability")
abline(0,1)

# Kolmogorov-Smirnov test of RAND variates against U(0, 1) distribution
ks.test(x,"punif",0,1)
```
...
*Yes, the pseudo-random numbers appear uniformly distributed because the histogram is fairly flat, and the empirical distribution function follows very closely to the line.*

* What are your conclusions from the Kolmogorov-Smirnov test at the $\alpha = 0.05$ level?

*At α = 0.05, we fail to reject the null hypothesis, meaning that the data is consistent with a uniform(0,1) distribution.*
...
for (i in 1:simnum) {
samp = sample(pop,3) # draw 3 marbles
blues[i] = sum(samp=="b") # number of blue marbles drawn
yellows[i] = sum(samp=="y") # number of yellow marbles drawn
}
table(blues)
table(yellows)
```
...
``` {r p1}
blue_pmf = prop.table(table(blues))
blue_pmf_df = data.frame(
  Blue = as.numeric(names(blue_pmf)),
  Empirical_PMF = as.numeric(blue_pmf)
)
blue_pmf_df = setNames(blue_pmf_df, c("Number of blue marbles drawn", "Empirical pmf"))
blue_pmf_df
```

* Present the empirical pmf of the number of yellow marbles drawn.
``` {r p2}
yellow_pmf = prop.table(table(yellows))
yellow_pmf_df = data.frame(
  yellow = as.numeric(names(yellow_pmf)),
  Empirical_PMF = as.numeric(yellow_pmf)
)
yellow_pmf_df = setNames(yellow_pmf_df, c("Number of yellow marbles drawn", "Empirical pmf"))
yellow_pmf_df
```

* Present the mean and variance of the number of blue marbles drawn.
``` {r p3}
cat("The mean of the number of blue marbles drawn is", mean(blues), "\n")
cat("The variance of the number of blue marbles drawn is", var(blues))
```

* Present the mean and variance of the number of yellow marbles drawn.
``` {r p4}
cat("The mean of the number of yellow marbles drawn is", mean(yellows), "\n")
cat("The variance of the number of yellow marbles drawn is", var(yellows))
```

* Compare the empirical pmf with the true values.
``` {r p5}
blue_compare = data.frame(
  Blue = c(0,1,2),
  Empirical = as.numeric(blue_pmf[c("0","1","2")]),
  True = c(56/120, 56/120, 8/120)
)
blue_compare = setNames(blue_compare, c("Number of blue marbles", "Empirical pmf", "True pmf"))
blue_compare

yellow_compare = data.frame(
  Yellow = c(0,1,2,3),
  Empirical = as.numeric(yellow_pmf[c("0","1","2","3")]),
  True = c(20/120, 60/120, 36/120, 4/120)
)
yellow_compare = setNames(yellow_compare, c("Number of yellow marbles", "Empirical pmf", "True pmf"))
yellow_compare
```
