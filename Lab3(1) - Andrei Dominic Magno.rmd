...
success = 0
numstudents = 50
n = 1000 #birthdays for 1000 classes
for(i in 1:n){
  trial = sample(1:365, numstudents, replace=TRUE)
  
  if(3 %in% table(trial)) {success[i] = 1} else {success[i] = 0}
}
...
# [Place code here]
  ...
  success = 0   #store successes
  
  for (i in 1:trials) {
    n = 52  # permutation of size n
  	perm = 1:n # store list of n integers
	  for(card in 1:(n-1)){
		  x = sample(card:n,1) # randomly chose a position for swapping
	...
	if(floor((perm[1]-1)/13) == floor((perm[52]-1)/13)) {success[i] = 1} else {success[i] = 0}
  }
  mean(success)
```
...
*The empirical probability that in a randomly shuffled deck, the top and bottom cards are the same suit is 0.246, which is similar to the theoretical probability of 0.235.*
...
*The code `(floor((perm[1]-1)/13) == floor((perm[52]-1)/13))` works by checking the first and last index of the permutation, and comparing the suits of the given number. By dividing by 13, you can divide the deck into 4 suits in which the floor function identifies which suit belongs to what card. Then, the == operator would check if the first and last card equal to the same suit.*
...
*Intuitively, since the first card of the permutation is already fixed, we know that the chance of the last card being the same suit as the first would be 12/51, which matches the probability of approximately 0.235. This value is very similar to the value observed in the simulation.*
