...
*I would expect around 2 because there is a one forth chance of each side.*
...
for(simnum in 1:S){
  ...
  roll = sample(x, 10, replace = TRUE)  # simulate a die roll
  rolls.avgs[simnum] = mean(roll) # store the average roll
}
# take a look at the first 6 simulation results
head(rolls.avgs)
...
*Yes, the mean of 1000 simulations would be closer to the expected value because the sample mean would approaches the expected value with increasing simulation size. *

•	How do you interpret the standard deviation in this problem?

*The standard deviation describes how the 1000 experiments vary around the mean.*
...
*The rolls vary from 1-4 in each replicate, showing that there is no clear pattern.*
...
mean(rolls.avgs)
hist(rolls.avgs, main="", xlab="Average of set of 10 rolls")
hist(rolls.avgs, prob = TRUE, main="", xlab="Average of set of 10 rolls") # histogram
curve(dnorm(x, mean=mean(rolls.avgs), sd=sd(rolls.avgs)), add=TRUE, col="green") # normal approximation
```
...
*The histogram-shape has a bell-shaped curve, symmetrically skewed, and the center is approximately 3 which is the expected average. The spread is moderate, where most averages fall near the center and the extreme cases are on the edges.*

•	Does this follow what you would expect to see?

*Yes, the distribution of sample averages tend to be normal and centered at the expected value as the number of simulations increases.*
...
First problem
How often in 5 rolls of a die is a two rolled?
...
*After running it 5 times, I gained the values {0,0,1,1,2}.*

•	Are the values different? Is that what you expect?

*The values are different which is expected because since each set of fives rolls is random, the number of times a 2 appears can change from each roll.*
...
# [Place code here]
count = 0 # start a counter for number of times at least one six shows in 5 rolls
S = 1000 # number of experiments
# for-loop to repeat die rolling experiment 1000 times
for(simnum in 1:S){
  x = 1:4 # 4-sided die
    roll = sample(x, 5, replace = TRUE) # roll the die five times
    count = count + (sum(roll==2)>0)  # Boolean computation: add one to the counter only if at least one 6 shows.
}
count/S
```
...
*Yes, the value I got was close to the truth.*

•	How can we modify the simulation experiment to get a value even close to the truth?

*We could increase the number of experiments to get a value even closer to the truth.*
